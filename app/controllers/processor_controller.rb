class ProcessorController < ApplicationController
  ...
  def oaithumbnail
    ...
    result = Resque.enqueue(Thumbnail, :object_pid=>params[:pid], :is_new=>is_new, :environment=>params[:environment], :system_type=>params[:system_type], :image_urls=>params[:image_urls], :thumbnail_url=>thumbnail_url)
